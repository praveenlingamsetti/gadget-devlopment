 ...
 */ export type StateHistoryValue = {
    /** The name of the state that was previously active */ current: string;
    /** Historical values of all nested states */ children?: Record<string, StateHistoryValue>;
};
/** Data about a state, as returned by lookup by state key via `StateMapper` */ export type StateData = {
    /** The state value for a given state, as we'd persist it in the DB */ value: StateValue;
    /** The API identifier for the state (can be changed by the user) */ apiIdentifier: string;
    /** The `StateBlob` this state was derived from, as given to the app sandbox */ blob: StateBlob;
    /** A path to key to this state from the root, useful with lodash's get/set */ path: string[];
};
/** Deeply map the keys/values of a state value */ declare function mapStateValue(state: StateValue, mapper: (key: string) => string): StateValue;
/** Deeply map the keys/values of a state history value */ declare function mapStateHistoryValue(state: StateHistoryValue, mapper: (key: string) => string): StateHistoryValue;
/** Determine if a given value is a state value */ export declare function isStateValue(value: any): value is StateValue;
/** Determine if a given value is a state history value */ export declare function isStateHistoryValue(value: any): value is StateHistoryValue;
/**
